...
export class ErrorInterceptor implements HttpInterceptor {
  ...
  badRequestNotification(error: HttpErrorResponse): void {
    ...
    if (
      error.error.errorMessage.indexOf(
        'Acesso Negado#Este usuário não tem permissão!'
      ) == -1 &&
      error.error.errorMessage.indexOf(
        'Situação#Não foram encontradas informações de situação parametrizadas'
      ) == -1 &&
      error.error.errorMessage.indexOf(
        'Retorna registros de horas extras não autorizadas para o colaborador selecionado": [ERRO] Usuario#Usuário não encontrado'
      ) == -1
    ) {
